...
function FuelHistoryPage() {
  ...
  const headers = ["date", "vehicle", "fuel_col_litres", "total"] as const;

  return (
    <div>
      <PageHeader title={t("fuel_history_title")} subtitle={t("total_spend", { amount: money(total) })} />
            ...
            {fuel.map((f) => {
              ...
              return (
                <tr key={f.id} className="border-t border-border-soft">
                  <td className="px-4 py-3">{shortDate(f.date)}</td>
