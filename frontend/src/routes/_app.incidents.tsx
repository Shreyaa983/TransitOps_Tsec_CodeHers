...
function IncidentsPage() {
  ...
  const loadIncidents = async () => {
    ...
    try {
      ...
      toast.error(e.message || t("incidents_load_failed"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleApprove = async (incident: any) => {
    ...
    try {
      ...
      if (res.success) {
        toast.success(t("incidents_approved"));
      ...
      toast.error(e.message || t("incidents_approve_failed"));
    } finally {
  ...
  const handleReject = async (id: string) => {
    ...
    try {
      ...
      if (res.success) {
        toast(t("incidents_rejected"));
        setIncidents((prev) => prev.filter((i) => i._id !== id));
      }
    } catch (e: any) {
      toast.error(e.message || t("incidents_reject_failed"));
    } finally {
      setIsProcessing(null);
    }
  };
  ...
  return (
    <div className="pb-20">
      <PageHeader
        title={t("incidents_title")}
        subtitle={t("incidents_subtitle", { count: incidents.length })}
        actions={
      ...
      <div className="space-y-4 max-w-3xl mt-6">
        {isLoading ? (
          <div className="p-8 text-center text-muted-foreground font-semibold">{t("incidents_loading")}</div>
        ) : incidents.length === 0 ? (
          <div className="brutal-card p-12 text-center flex flex-col items-center justify-center">
            ...
            <p className="font-extrabold text-xl text-foreground">{t("incidents_all_caught_up")}</p>
            <p className="text-muted-foreground text-sm font-semibold mt-1">{t("incidents_no_pending")}</p>
          </div>
        ) : (
          incidents.map((n) => (
            <div key={n._id} className="w-full text-left brutal-card p-5 border-2 border-danger/40 bg-danger/5">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2 text-danger font-black uppercase tracking-widest text-sm">
                  <AlertTriangle className="w-4 h-4 text-danger animate-pulse" /> {t("incidents_pending_review")}
                </div>
                <div className="text-xs font-bold text-muted-foreground flex items-center gap-1">
                  <Clock className="w-3 h-3" /> {shortDate(n.createdAt)}
                </div>
              </div>
              ...
              <div className="bg-background rounded-xl p-4 border border-border-soft mb-4 text-sm font-medium leading-relaxed whitespace-pre-wrap brutal-shadow-sm">
                <span className="font-bold uppercase tracking-wider text-muted-foreground text-[10px] block mb-1">{t("incidents_driver_obs")}</span>
                {n.observation}
              </div>

              <div className="bg-muted/30 rounded-xl p-4 border border-border-soft mb-5 space-y-2">
                <p className="text-[11px] font-black uppercase text-muted-foreground tracking-widest mb-2">{t("incidents_ai_output")}</p>
                ...
                <p className="text-xs font-semibold">{t("incidents_summary")} <span className="font-medium text-muted-foreground">{n.aiAnalysis?.summary}</span></p>
                <p className="text-xs font-semibold">{t("incidents_recommended")} <span className="font-medium text-muted-foreground">{n.aiAnalysis?.recommendedAction}</span></p>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                >
                  <Wrench className="w-4 h-4" /> {isProcessing === n._id ? t("incidents_approving") : t("incidents_approve")}
                </button>
                <button
                  disabled={isProcessing === n._id}
