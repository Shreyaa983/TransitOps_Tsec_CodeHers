...
function ReportIncidentPage() {
    ...
    const chipKeys: I18nKey[] = [
        "incident_chip_stopped", "incident_chip_smoke", "incident_chip_noise",
        "incident_chip_brake", "incident_chip_steering", "incident_chip_warning",
    ];

    const handleAnalyze = async () => {
        if (!incidentText.trim()) return toast.error(t("incident_describe_first"));
        setIsAnalyzing(true);
        setAiReport(null);
        setIsSuccess(false);
        ...
        const interval = setInterval(() => {
            step++;
            if (step < loadingStepKeys.length) {
        ...
        try {
            ...
            if (res.success) {
                ...
                toast.error(t("incident_analyze_failed"));
            }
        } catch (e: any) {
            clearInterval(interval);
            toast.error(e.message || t("incident_analyze_failed"));
        } finally {
            clearInterval(interval);
    ...
    const handleSubmit = async () => {
        if (!aiReport || !assignedVehicle) return toast.error(t("incident_missing_report"));
        ...
        try {
            ...
            toast.error(e.message || t("incident_submit_failed"));
        } finally {
        ...
        <div className="max-w-3xl mx-auto space-y-6 pb-20">
            ...
            {isSuccess && (
                <div className="brutal-card p-10 bg-card border-success border-2 flex flex-col items-center justify-center text-center space-y-3 animate-in fade-in zoom-in duration-500">
                    ...
                    <h2 className="text-3xl font-black text-foreground tracking-tight">{t("incident_submitted")}</h2>
                    <p className="text-base font-bold text-muted-foreground">{t("incident_submitted_body")}</p>
                    <p className="text-sm font-black text-success tracking-widest uppercase mt-4 border border-success/30 bg-success/5 px-4 py-2 rounded-lg brutal-shadow-sm">
                        {t("incident_reference", { year: new Date().getFullYear(), number: Math.floor(100 + Math.random() * 900) })}
                    </p>
                </div>
            )}
                ...
                <div className="flex items-center justify-between mb-4">
                    {assignedVehicle ? (
                        <div className="bg-muted/30 border border-border-soft rounded-lg px-5 py-4 w-full">
                            <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-1.5">{t("incident_reporting_for")}</p>
                            <div className="flex items-center gap-3">
                                <span className="text-2xl">🚐</span>
                                <div>
                                    <p className="font-black text-xl leading-tight">{assignedVehicle.name}</p>
                                    <p className="text-sm font-semibold text-muted-foreground mt-0.5">{t("incident_registration")} <span className="font-mono text-foreground">{assignedVehicle.registration}</span></p>
                                </div>
                            </div>
                        </div>
                    ) : (
                        <div className="bg-warning/10 border border-warning/30 rounded-lg px-5 py-4 w-full text-warning-foreground text-sm font-semibold text-center">
                            {t("incident_no_vehicle")}
                        </div>
                    )}
                </div>

                <div className="space-y-4">
                    <label className="text-sm font-black uppercase tracking-wider text-foreground pl-1">{t("incident_describe")}</label>
                    <textarea
                        value={incidentText}
                    ...
                    {!aiReport && (
                        ...
                        >
                            {isAnalyzing ? t(loadingStepKeys[loadingStepIndex]) : t("incident_create_report")}
                        </button>
                    )}
                </div>
            </div>
                ...
                <div className="border-2 border-border-soft bg-card rounded-xl overflow-hidden shadow-lg animate-in slide-in-from-bottom-4 fade-in duration-500">
                    <div className="bg-muted/10 p-5 border-b border-border-soft flex items-center justify-between">
                        <div className="flex items-center gap-2 text-primary font-black tracking-widest uppercase text-[11px]">
                            <CheckCircle2 className="w-4 h-4" /> {t("incident_ai_complete")}
                        </div>
                    </div>
                    ...
                    <div className="p-6">
                        <div className="flex items-start justify-between flex-wrap gap-4 mb-6">
                            ...
                            <div className="flex gap-3 items-center">
                                {!aiReport.dispatchAllowed && <span className="text-xs uppercase font-black text-danger border border-danger/40 bg-danger/10 px-3 py-1.5 rounded flex items-center gap-1.5"><AlertTriangle className="w-3.5 h-3.5" /> {t("incident_no_dispatch")}</span>}
                                {aiReport.requiresImmediateStop && <span className="text-xs uppercase font-black text-danger border border-danger bg-danger/20 px-3 py-1.5 rounded flex items-center gap-1.5"><AlertTriangle className="w-3.5 h-3.5" /> {t("incident_immediate_stop")}</span>}
                            </div>
                        </div>

                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                            <div className="bg-muted/20 rounded-lg p-4 border border-border-soft">
                                <p className="text-[10px] uppercase font-bold text-muted-foreground mb-1 tracking-wider">{t("incident_ai_confidence")}</p>
                                <p className="font-black text-xl">{(aiReport.confidence * 100).toFixed(0)}%</p>
                            </div>
                            <div className="bg-muted/20 rounded-lg p-4 border border-border-soft">
                                <p className="text-[10px] uppercase font-bold text-muted-foreground mb-1 tracking-wider">{t("incident_est_downtime")}</p>
                            ...
                            <div className="col-span-2 bg-muted/20 rounded-lg p-4 border border-border-soft">
                                <p className="text-[10px] uppercase font-bold text-muted-foreground mb-1 tracking-wider">{t("incident_likely_parts")}</p>
                        ...
                        <div className="space-y-4">
                            <div className="bg-background rounded-xl p-5 border border-border-soft brutal-shadow-sm">
                                <p className="font-bold text-foreground mb-2 uppercase text-[11px] tracking-widest text-muted-foreground">{t("incident_tech_summary")}</p>
                            ...
                            <div className="bg-warning/10 rounded-xl p-5 border border-warning/30 brutal-shadow-sm">
                                <p className="font-bold mb-2 uppercase text-[11px] tracking-widest text-warning">{t("incident_recommended_action")}</p>
                    ...
                    <div className="p-6 border-t border-border-soft bg-muted/10">
                        <p className="text-[11px] font-black uppercase tracking-widest text-center text-muted-foreground mb-4">{t("incident_review_report")}</p>
                        <button
                        ...
                        >
                            {isSubmitting ? t("incident_submitting") : t("incident_submit_official")}
                        </button>
                    </div>
                </div>
            )}
