...
function EditTrip() {
  ...
  if (!trip || !form) return <EmptyState title={t("trips_not_found")} />;

  return (
    <div>
      <PageHeader title={t("trips_edit_title", { code: trip.code })} />
      <form
        onSubmit={(e) => {
          e.preventDefault();
      ...
      >
        ...
        <F label={t("trips_cargo_kg")}><Input type="number" className="brutal-input" value={form.cargoKg} onChange={(e) => setForm({ ...form, cargoKg: +e.target.value })} /></F>
        ...
        <div className="md:col-span-2 flex justify-end gap-2 pt-2">
          <Button type="button" variant="outline" className="brutal-btn bg-card" onClick={() => nav({ to: "/trips/$tripId", params: { tripId: trip.id } })}>{t("cancel")}</Button>
          <Button type="submit" className="brutal-btn bg-primary text-primary-foreground">{t("save")}</Button>
        </div>
      </form>
    </div>
  );
