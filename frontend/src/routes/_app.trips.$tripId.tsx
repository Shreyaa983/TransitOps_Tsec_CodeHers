        ...
        actions={
          <div className="flex gap-2">
            ...
            {trip.status === "dispatched" && (
              <Link to="/trips/$tripId/complete" params={{ tripId: trip.id }} className="brutal-btn px-3 py-2 bg-success text-success-foreground inline-flex items-center gap-1">
                <CheckCircle2 className="h-4 w-4" /> {t("complete")}
              </Link>
            )}
            {(trip.status === "draft" || trip.status === "dispatched") && !isDriver && (
              <Button variant="outline" onClick={async () => { await cancelTrip(trip.id); toast.success(t("trips_cancelled")); }} className="brutal-btn bg-card"><XCircle className="h-4 w-4 mr-1" /> {t("status_cancelled")}</Button>
            )}
          </div>
        }
      />
